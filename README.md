...
Sloppy Butchery is a collection of audio tools for [Google Colaboratory](https://colab.research.google.com/) (i.e. your browser), using [you Google Drive](https://drive.google.com/drive/my-drive) as data source and storage.
...
1. **Drum track extraction for timing:** Stem track of drums is extracted from input audio by [Spleeter](https://github.com/deezer/spleeter) convolutional neural network to be used as a _timing track_. Timing track is used for initial estimation of beat positions, i.e. input audio track (audiofile#1) is eventually sliced according to analysis performed on the timing track (audiofile#2).
